...
#endif
	/* technically the term has a 1->N with the z80 including null */
	/* arbitration should be here for mutli task */
	z80term = z80;
}
...
loadfile(z80info *z80, const char *fname)
{
	...
	/* unlikely but 200 < 256 so */
	/* max name plus extension of 3 plus dot plus null byte */
	if(strlen(fname) >= sizeof(buf) - 5) return FALSE; 
	...
	/* hook point for loading and processing some other file types */
